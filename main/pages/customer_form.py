...
def init_db():
    """Ensure the SQLite database table matches your schema."""
...
for key in ["show_resolved_popup", "complaint_submitted", "response", "category", "chat_ready", "view_complaints"]:
    if key not in st.session_state:
        st.session_state[key] = False if key not in ["response", "category"] else ""
...
# --- Insert Complaint to DB (SQLite Version) ---
def insert_complaint_sqlite(customer_id, complaint, category, response, urgency, followup):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
with st.form("complaint_form"):
    ...
    if submitted:
        ...
        insert_complaint_sqlite(customer_id, complaint_text, category, response, urgency, "")
...
if st.session_state.complaint_submitted:
    ...
    if follow_up:
        ...
        # SQLite syntax: Order by Id descending to update the latest complaint
